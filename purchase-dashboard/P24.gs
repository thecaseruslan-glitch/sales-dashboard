...
const SHEET_STOCKOUT_LOG = 'stockout_log';
...
const PURCHASES_SESSION_CACHE_PREFIX = 'purchases_session:';
const PURCHASES_SESSION_TTL_SECONDS = 6 * 60 * 60;
...
function RUN_01_setupSheets() {
  const reports = [
    ...
    ensureSystemSheet(SHEET_STOCKOUT_LOG, STOCKOUT_LOG_HEADERS),
    ensureSystemSheet(SHEET_SYSTEM_META, SYSTEM_META_HEADERS),
    ensureSystemSheet(SHEET_ACCESS_CONTROL, ACCESS_CONTROL_HEADERS)
  ];
...
function RUN_02_getSetupState() {
  ...
  const expected = [
    ...
    { sheet: SHEET_ACCESS_CONTROL, headers: ACCESS_CONTROL_HEADERS }
  ];
...
function normalizeRole_(value) {
  const role = normalizeCell(value).toLowerCase();
  if (role === 'admin' || role === 'адмін' || role === 'administrator') return 'admin';
  if (role === 'manager' || role === 'менеджер' || role.indexOf('менеджер') >= 0) return 'manager';
  return role || 'manager';
}
...
function buildPurchasesSessionToken_() {
  return Utilities.getUuid() + ':' + sha256Hex_(String(Date.now()) + ':' + Math.random()).slice(0, 16);
}
...
function getAccessControlSheet_() {
  ...
  applyTextFormatsForHeaders_(sheet, ACCESS_CONTROL_HEADERS);
  return sheet;
}

function getAccessControlRows_() {
  getAccessControlSheet_();
  return getSheetObjects_(SHEET_ACCESS_CONTROL, ACCESS_CONTROL_HEADERS)
    .map(function(row) {
      return {
        email: normalizeCell(row.email).toLowerCase(),
        display_name: normalizeCell(row.display_name),
        role: normalizeRole_(row.role),
        active: normalizeCell(row.active),
        password_hash: normalizeCell(row.password_hash),
        temp_password: normalizeCell(row.temp_password),
        updated_at: normalizeCell(row.updated_at)
      };
    })
    .filter(function(row) {
      return row.email && isTruthyCell(row.active);
    });
}

function findAccessUserByEmail_(email) {
  const cleanEmail = normalizeCell(email).toLowerCase();
  if (!cleanEmail) return null;
  const rows = getAccessControlRows_();
  for (var i = 0; i < rows.length; i++) {
    if (rows[i].email === cleanEmail) return rows[i];
  }
  return null;
}

function toClientUser_(access) {
  if (!access) return null;
  const role = normalizeRole_(access.role);
  return {
    email: access.email,
    display_name: access.display_name || access.email,
    role: role,
    is_admin: role === 'admin',
...
function cachePurchaseSession_(token, access) {
  const user = toClientUser_(access);
  if (!token || !user) throw new Error('Не вдалося створити сесію');
  CacheService.getScriptCache().put(
    PURCHASES_SESSION_CACHE_PREFIX + token,
    JSON.stringify(user),
    PURCHASES_SESSION_TTL_SECONDS
  );
  return user;
}

function getPurchaseSessionUser_(sessionToken) {
  const token = normalizeCell(sessionToken);
  if (!token) return null;
  const raw = CacheService.getScriptCache().get(PURCHASES_SESSION_CACHE_PREFIX + token);
  if (!raw) return null;
  try {
...
function requirePurchasesUserBySession_(sessionToken) {
  const user = getPurchaseSessionUser_(sessionToken);
  if (!user || !user.email) {
    throw new Error('Потрібно увійти в дашборд закупок');
  }
  return user;
}

function requirePurchasesAdminBySession_(sessionToken) {
  const user = requirePurchasesUserBySession_(sessionToken);
  if (!user.is_admin) {
...
function serverLogin(email, password) {
  ...
  if (!cleanEmail || !cleanPassword) throw new Error('Вкажіть логін і пароль');

  const access = findAccessUserByEmail_(cleanEmail);
  if (!access) throw new Error('Користувача не знайдено або доступ вимкнений');

  if (access.password_hash) {
    if (sha256Hex_(cleanPassword) !== access.password_hash) {
      throw new Error('Невірний пароль');
    }
  } else if (access.temp_password) {
    if (cleanPassword !== access.temp_password) {
      throw new Error('Невірний пароль');
    }
  } else {
    throw new Error('Для користувача не задано пароль');
  }

  const sessionToken = buildPurchasesSessionToken_();
  const user = cachePurchaseSession_(sessionToken, access);
...
function serverLogout(sessionToken) {
  const token = normalizeCell(sessionToken);
  if (token) {
    CacheService.getScriptCache().remove(PURCHASES_SESSION_CACHE_PREFIX + token);
  }
  return { ok: true };
}

function hashTempPasswordsInAccessControl() {
  const sheet = getAccessControlSheet_();
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return { ok: true, updated: 0 };

  const values = sheet.getRange(2, 1, lastRow - 1, ACCESS_CONTROL_HEADERS.length).getValues();
  const passwordHashCol = ACCESS_CONTROL_HEADERS.indexOf('password_hash');
  const tempPasswordCol = ACCESS_CONTROL_HEADERS.indexOf('temp_password');
  const updatedAtCol = ACCESS_CONTROL_HEADERS.indexOf('updated_at');
  let updated = 0;

  for (var i = 0; i < values.length; i++) {
    const tempPassword = normalizeCell(values[i][tempPasswordCol]);
    const passwordHash = normalizeCell(values[i][passwordHashCol]);
    if (!tempPassword || passwordHash) continue;
    values[i][passwordHashCol] = sha256Hex_(tempPassword);
    values[i][tempPasswordCol] = '';
    values[i][updatedAtCol] = nowText();
    updated++;
  }

  if (updated) {
    sheet.getRange(2, 1, values.length, ACCESS_CONTROL_HEADERS.length).setValues(values);
  }
...
function isTruthyCell(value) {
  ...
  return text === 'true' || text === '1' || text === 'yes' || text === 'y' || text === 'так' || text === 'active' || text === 'активний';
}
...
function buildStockManualKey_(row) {
  ...
  const code = normalizeProductCode_(row && row.code).toUpperCase();
...
function writeRowsToSheet_(sheetName, headers, rows) {
  const sheet = getOrCreateSheet(sheetName);
  const normalizedRows = normalizeRowsForSheetWrite_(headers, rows);
  sheet.clearContents();
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  ...
  return normalizedRows ? normalizedRows.length : 0;
}

function normalizeRowsForSheetWrite_(headers, rows) {
  if (!Array.isArray(headers) || !Array.isArray(rows) || !rows.length) return rows || [];
  const codeIndex = headers.indexOf('code');
  if (codeIndex < 0) return rows;

  return rows.map(function(row) {
    const normalized = Array.isArray(row) ? row.slice() : row;
    if (Array.isArray(normalized)) normalized[codeIndex] = normalizeProductCode_(normalized[codeIndex]);
    return normalized;
  });
}
...
function getSheetObjects_(sheetName, headers) {
  ...
  return values.map(function(row) {
    ...
    sourceHeaders.forEach(function(header, index) {
      item[header] = header === 'code' ? normalizeProductCode_(row[index]) : row[index];
    });
    return item;
  });
}
...
function buildStockCurrentRows_(stockRows, assortmentRows, manualValues) {
  ...
  return Object.keys(stockById).map(function(id) {
    ...
    const code = normalizeProductCode_(extractCode_(entity));
    const manual = manualMap['id::' + id] || manualMap['code::' + normalizeProductCode_(code).toUpperCase()] || {};
...
function refreshStockCurrentNow_() {
  ...
  const stockout = recordStockoutSnapshot_(rows);
  ...
  return {
    ...
    stockout_rows_recorded: stockout.recorded,
    completed_at: nowText()
  };
}

function recordStockoutSnapshot_(stockRows) {
  const rows = Array.isArray(stockRows) ? stockRows : [];
  const sheet = getOrCreateSheet(SHEET_STOCKOUT_LOG);
  ensureHeadersForSheet(sheet, STOCKOUT_LOG_HEADERS);
  applyTextFormatsForHeaders_(sheet, STOCKOUT_LOG_HEADERS);

  const today = formatDateOnly_(new Date());
  const existing = {};
  const lastRow = sheet.getLastRow();
  if (lastRow > 1) {
    const values = sheet.getRange(2, 1, lastRow - 1, STOCKOUT_LOG_HEADERS.length).getValues();
    const dateIdx = STOCKOUT_LOG_HEADERS.indexOf('date');
    const codeIdx = STOCKOUT_LOG_HEADERS.indexOf('code');
    values.forEach(function(row) {
      const date = normalizeCell(row[dateIdx]).slice(0, 10);
      const code = normalizeProductCode_(row[codeIdx]);
      if (date && code) existing[date + '::' + code] = true;
    });
  }

  const out = [];
  rows.forEach(function(row) {
    const code = normalizeProductCode_(row[STOCK_CURRENT_HEADERS.indexOf('code')]);
    if (!code) return;
    const stock = Number(row[STOCK_CURRENT_HEADERS.indexOf('stock')] || 0);
    const inTransit = Number(row[STOCK_CURRENT_HEADERS.indexOf('expected')] || 0);
    if (stock > 0) return;
    ...
    out.push([
      today,
      code,
      normalizeCell(row[STOCK_CURRENT_HEADERS.indexOf('product')]),
      normalizeCell(row[STOCK_CURRENT_HEADERS.indexOf('brand')]),
      normalizeCell(row[STOCK_CURRENT_HEADERS.indexOf('category')]),
      normalizeCell(row[STOCK_CURRENT_HEADERS.indexOf('group')]),
      round2(stock),
      round2(inTransit),
      'TRUE',
      nowText()
  ...
  if (out.length) {
    sheet.getRange(sheet.getLastRow() + 1, 1, out.length, STOCKOUT_LOG_HEADERS.length).setValues(out);
  }
  return { ok: true, recorded: out.length };
}
...
function updateStockItemManualStatus(sessionToken, payload) {
  requirePurchasesAdminBySession_(sessionToken);
  const source = payload && typeof payload === 'object' ? payload : {};
  const productId = normalizeCell(source.product_id);
  const sku = normalizeProductCode_(source.sku || source.code);
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    const rowCode = normalizeProductCode_(values[i][codeColumn - 1]);
...
function RUN_26_setupAccessControl() {
  return ensureSystemSheet(SHEET_ACCESS_CONTROL, ACCESS_CONTROL_HEADERS);
}

function RUN_27_hashTempPasswordsInAccessControl() {
...
function fetchMissingLinkedPurchaseOrderDocsFromReceiptRows_(receiptRows, purchaseRows) {
  ...
  (Array.isArray(purchaseRows) ? purchaseRows : []).forEach(function(row) {
    ...
    const productPart = (
      ...
      normalizeProductCode_(row.code).toUpperCase() ||
...
function buildPurchaseOrderRowKeyFromParts_(idx, row) {
  ...
  const code = normalizeProductCode_(row[idx.code]).toUpperCase();
...
function buildSalesHistoryRowKey_(row) {
  ...
  const sku = normalizeProductCode_(row[idx.code] || '');
...
function getPurchasesDashboardData() {
  ...
  const stockoutRows = getSheetObjects_(SHEET_STOCKOUT_LOG, STOCKOUT_LOG_HEADERS);
...
function buildPurchaseAnalysisRows_(purchaseOrders, stock) {
  ...
  return sourceRows
    ...
    .map(function(row) {
      ...
      return {
        order_name: normalizeCell(row.order_name),
        code: normalizeProductCode_(row.code),
...
function buildProductAnalysisRows_(stock, purchaseOrders, salesHistory, settingsRows) {
  ...
  function ensureRow(seed) {
    ...
    if (!byProduct[key]) {
      ...
      byProduct[key] = {
        ...
        code: normalizeProductCode_(seed.code),
      ...
      row.code = row.code || normalizeProductCode_(seed.code);
  ...
  (Array.isArray(salesHistory) ? salesHistory : []).forEach(function(row) {
    ...
    const shipmentKey = normalizeCell(row.order_id || row.order_name) || (date + '::' + normalizeCell(row.position_id || normalizeProductCode_(row.code) || row.product));
...
function savePurchaseAnalysisSettingForClient(sessionToken, payload) {
  requirePurchasesAdminBySession_(sessionToken);
  const source = payload && typeof payload === 'object' ? payload : {};
  const productId = normalizeCell(source.product_id);
  const code = normalizeProductCode_(source.code);
...
function normalizePurchaseAnalysisManualChange_(payload) {
  ...
  const out = {
    ...
    sku: normalizeProductCode_(source.sku || source.code),
    code: normalizeProductCode_(source.code || source.sku),
...
function savePurchaseAnalysisManualBatchForClient(sessionToken, payload) {
  requirePurchasesAdminBySession_(sessionToken);
  const source = payload && typeof payload === 'object' ? payload : {};
  ...
  changes.forEach(function(raw, index) {
    ...
    try {
      if (hasStockChange) {
        updateStockItemManualStatus(sessionToken, change);
        stockUpdates++;
      }
      if (hasAnalysisChange) {
        savePurchaseAnalysisSettingForClient(sessionToken, change);
        analysisUpdates++;
      }
    } catch (error) {
...
function buildPurchasesFrontendPayload_(data, user) {
  ...
  return {
    ...
    user: currentUser ? toClientUser_(currentUser) : null,
    meta: source.meta || {},
...
function getPurchasesRefreshLoopSignalForClient(sessionToken) {
  requirePurchasesUserBySession_(sessionToken);
  return {
    ok: true,
    generated_at: nowText(),
    refresh_loop: getRefreshLoopState(),
...
function buildPurchasesDashboardRows_(stock, purchaseOrders, receipts) {
  ...
  function ensureRow(seed) {
    ...
    item.code = item.code || normalizeProductCode_(seed.code);
...
function getPurchasesFrontendPayloadForClient(sessionToken) {
  const user = requirePurchasesUserBySession_(sessionToken);
  return buildPurchasesFrontendPayload_(null, user);
}
